"""
cache.py — универсальный парсер: кеш + бэктест + summary за один запуск.

Требования тимлида:
  ✅ Ограничение по времени исполнения (24 часа) — скрипт сам остановится
  ✅ Идемпотентность — можно остановить и запустить заново, продолжит с места остановки
  ✅ После завершения — только сообщение, никаких лишних действий
  ✅ INSERT IGNORE везде — защита от двойной записи при параллельных запросах
  ✅ Трассировка на почту по каждому этапу (кеш завершён / бэктест завершён / ошибка)

Запуск:
    python cache.py <table_name> <host> <port> <user> <password> <database> <model_id>

Пример:
    python cache.py vlad_data_bot localhost 3307 vlad W7ReR%HRLXLJTZC7 vlad 33

Флаги:
    --date-from YYYY-MM-DD    по умолчанию: 2025-01-15
    --date-to   YYYY-MM-DD    по умолчанию: сегодня
    --pair  1 3 4             по умолчанию: все
    --day   0 1               по умолчанию: оба
    --tier  0 1               по умолчанию: оба
    --skip-fill               пропустить заполнение кеша
    --only-fill               только кеш, бэктест не запускать
    --timeout-hours N         лимит времени в часах (по умолчанию: 24)
"""
...
Примеры:
  ...
  python cache.py vlad_data_bot localhost 3307 vlad pass vlad 33 --timeout-hours 12
    ...
    parser.add_argument("model_id",   type=int, help="ID модели из brain_service")
